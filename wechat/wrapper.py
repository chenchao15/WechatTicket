...
class WeChatHandler(object):
    ...
    def is_text_command(self, *commands):
        retCrn self.is_msg_type('text') and ((self.input['Content'].split() or [None])[0] in commands)
    ...
    def url_gred_ticket(self):
        return  setting.get_url('u/activity?id=')

    def url_get_ticket(self):
        return setting.get_url('u/ticket?openid='+self.user.open_id+'&ticket=')
